...
class PerceptionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("decision_events_topic_v2", "/adas/perception/decision_events")
        self.declare_parameter("route_constraints_topic_v2", "/adas/perception/route_constraints")
        ...
        self.declare_parameter("sign_temporal_vote_enabled", env_bool("SIGN_TEMPORAL_VOTE_ENABLED", True))
        self.declare_parameter("sign_temporal_min_votes", env_int("SIGN_TEMPORAL_MIN_VOTES", 2))
        self.declare_parameter("sign_temporal_window_s", env_float("SIGN_TEMPORAL_WINDOW_S", 1.2))
        self.declare_parameter("sign_temporal_track_px", env_float("SIGN_TEMPORAL_TRACK_PX", 90.0))
        ...
        self.decision_events_topic_v2 = self.get_parameter("decision_events_topic_v2").value
        self.route_constraints_topic_v2 = self.get_parameter("route_constraints_topic_v2").value
        ...
        self.sign_temporal_vote_enabled = bool(self.get_parameter("sign_temporal_vote_enabled").value)
        self.sign_temporal_min_votes = max(1, int(self.get_parameter("sign_temporal_min_votes").value))
        self.sign_temporal_window_s = max(0.1, float(self.get_parameter("sign_temporal_window_s").value))
        self.sign_temporal_track_px = max(8.0, float(self.get_parameter("sign_temporal_track_px").value))
        ...
        self.decision_events_pub_v2 = self.create_publisher(
            String,
            self.decision_events_topic_v2,
            10,
        )
        self.route_constraints_pub_v2 = self.create_publisher(
            String,
            self.route_constraints_topic_v2,
            10,
        )
        ...
        self.get_logger().info(f"decision_events_topic_v2={self.decision_events_topic_v2}")
        self.get_logger().info(f"route_constraints_topic_v2={self.route_constraints_topic_v2}")
        ...
        self.get_logger().info(
            "sign_temporal_vote "
            f"enabled={self.sign_temporal_vote_enabled} "
            f"min_votes={self.sign_temporal_min_votes} "
            f"window_s={self.sign_temporal_window_s} "
            f"track_px={self.sign_temporal_track_px}"
        )
    ...
    def _sign_bbox_center(self, det):
        bbox = det.get("bbox") or []
        try:
            x1, y1, x2, y2 = [float(v) for v in bbox[:4]]
            return (0.5 * (x1 + x2), 0.5 * (y1 + y2), max(1.0, x2 - x1), max(1.0, y2 - y1))
        except Exception:
            return None

    def update_sign_temporal_vote(self, det):
        if not isinstance(det, dict) or det.get("label") != "traffic_sign":
            return det

        now = time.time()
        sign_type = self.normalize_text(det.get("sign_type", "unknown"))
        center = self._sign_bbox_center(det)
        if center is None:
            det["temporal_stable"] = False
            det["stable_count"] = 0
            det["sign_vote_debug"] = "SIGN_TEMPORAL_REJECT_UNSTABLE:no_bbox"
            return det

        if not self.sign_temporal_vote_enabled:
            det["temporal_stable"] = True
            det["stable_sign_type"] = sign_type
            det["stable_count"] = self.sign_temporal_min_votes
            det["sign_vote_debug"] = "SIGN_TEMPORAL_VOTE:disabled_accept"
            return det

        cx, cy, bw, bh = center
        window_s = float(self.sign_temporal_window_s)
        max_age = max(window_s * 2.0, 1.0)
        self.sign_vote_tracks = [
            tr for tr in self.sign_vote_tracks
            if now - float(tr.get("last_seen", 0.0)) <= max_age
        ]

        best = None
        best_dist = None
        for tr in self.sign_vote_tracks:
            tcx, tcy = tr.get("center", (cx, cy))
            dist_px = ((float(tcx) - cx) ** 2 + (float(tcy) - cy) ** 2) ** 0.5
            gate = max(float(self.sign_temporal_track_px), 0.75 * max(bw, bh, tr.get("size", 0.0)))
            if dist_px <= gate and (best_dist is None or dist_px < best_dist):
                best = tr
                best_dist = dist_px
        ...
        if best is None:
            ...
            self.sign_vote_tracks.append(best)

        try:
            conf = float(det.get("sign_confidence", det.get("confidence", 0.0)) or 0.0)
        except Exception:
            conf = 0.0
        ...
        best["last_seen"] = now
        best_votes = [
            vote for vote in best.get("votes", [])
            if now - float(vote.get("stamp", 0.0)) <= window_s
        ]
        best_votes.append({"stamp": now, "sign_type": sign_type, "confidence": conf})
        best["votes"] = best_votes

        counts = {}
        conf_sums = {}
        for vote in best_votes:
            vt = self.normalize_text(vote.get("sign_type", "unknown"))
            if not vt or vt == "unknown":
                continue
            counts[vt] = counts.get(vt, 0) + 1
            conf_sums[vt] = conf_sums.get(vt, 0.0) + float(vote.get("confidence", 0.0) or 0.0)

        if counts:
            stable_type, stable_count = max(
                counts.items(),
                key=lambda item: (item[1], conf_sums.get(item[0], 0.0)),
            )
            stable_conf = conf_sums.get(stable_type, 0.0) / max(1, stable_count)
        else:
            stable_type, stable_count, stable_conf = "unknown", 0, 0.0

        temporal_stable = bool(
            stable_type != "unknown"
            and stable_count >= int(self.sign_temporal_min_votes)
        )
        det["temporal_stable"] = temporal_stable
        det["stable_sign_type"] = stable_type
        det["stable_count"] = int(stable_count)
        det["temporal_vote_total"] = len(best_votes)
        det["temporal_vote_confidence"] = round(float(stable_conf), 4)
        det["sign_vote_debug"] = (
            f"SIGN_TEMPORAL_VOTE:type={sign_type},stable={stable_type},"
            f"count={stable_count}/{self.sign_temporal_min_votes},"
            f"total={len(best_votes)},stable_ok={int(temporal_stable)}"
        )

        return det

    def stabilized_route_sign_detection(self, det):
        det = self.update_sign_temporal_vote(det)
        if not self.sign_temporal_vote_enabled:
            return det
        if not det.get("temporal_stable", False):
            self.get_logger().info(
                "SIGN_TEMPORAL_REJECT_UNSTABLE "
                f"type={det.get('sign_type')} stable={det.get('stable_sign_type')} "
                f"count={det.get('stable_count')}/{self.sign_temporal_min_votes} "
                f"bbox={det.get('bbox')}"
            )
            return None

        stable = dict(det)
        stable_type = stable.get("stable_sign_type") or stable.get("sign_type")
        stable["sign_type"] = stable_type
        stable["sign_name"] = self.pretty_sign_name(stable_type)
        stable["sign_confidence"] = max(
            float(stable.get("sign_confidence", 0.0) or 0.0),
            float(stable.get("temporal_vote_confidence", 0.0) or 0.0),
        )
        stable["sign_source"] = f"{stable.get('sign_source', 'classifier')}|temporal_vote"
        self.get_logger().info(stable.get("sign_vote_debug", "SIGN_TEMPORAL_VOTE"))
        return stable
    ...
    def build_semantic_outputs(self, detections, tl_info, frame_w, frame_h):
        ...
        for det in detections:
            ...
            stable_route_det = self.stabilized_route_sign_detection(det)
            route_constraint = build_route_constraint_from_detection(stable_route_det, frame_w, frame_h) if stable_route_det is not None else None
    ...
    def publish_semantic_outputs(self, decision_events, route_constraints, frame_w, frame_h, tl_info):
        ...
        decision_payload = {
            ...
            "debug": {
                "schema": "perception_decision_events_v1",
            },
        }
        ...
        route_payload = {
            ...
            "debug": {
                "schema": "perception_route_constraints_v1",
            },
        }
