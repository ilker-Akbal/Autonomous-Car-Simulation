...
class CarlaControlAdapterNode(Node):
    ...
    def decision_callback(self, msg):
        try:
            ...
            # Explicit should_stop flag from Decision contract
            try:
                self.current_should_stop = bool(data.get("should_stop", False))
            except Exception:
                self.current_should_stop = False
    ...
    def control_loop(self):
        ...
        # Enforce Decision contract: any explicit should_stop OR STOP decision OR timeout
        # must override local PID/smoothing and produce zero throttle / full (safe) brake.
        effective_should_stop = (
            decision == "STOP"
            or bool(getattr(self, "current_should_stop", False))
            or decision_age > 2.0
        )
        ...
        if effective_should_stop:
            ...
            self.integral_error = 0.0
            # ensure target_speed can't be accidentally positive when stop is enforced
            target_speed = 0.0
