...
class DecisionNode(Node):
    ...
    def build_rule_decision(
        ...
        if traffic_light_state == "red":
            ...
            # TL_STOPLINE_FIX:
            # Sensör/stopline mesafesi yokken overhead ışıkta bbox aşağı inmiyor;
            # ışık üstte kalıyor ama bbox yavaş büyüyor. Eski eşikler logdaki
            # h=0.064,a=0.00076 değerinde çok erken STOP veriyordu.
            no_sensor_overhead_or_near_stop = (
                (
                    height_ratio is not None
                    and area_ratio is not None
                    and height_ratio >= 0.088
                    and area_ratio >= 0.00125
                )
                or (
                    height_ratio is not None
                    and area_ratio is not None
                    and bottom_ratio is not None
                    and bottom_ratio <= 0.18
                    and height_ratio >= 0.082
                    and area_ratio >= 0.00110
                )
                or (
                    height_ratio is not None
                    and height_ratio >= 0.110
                )
                or (
                    area_ratio is not None
                    and area_ratio >= 0.00190
                )
            )
        ...
        if traffic_light_state == "green":
            ...
            # TL_FIX:
            # Yeşil ışık sadece kırmızı latch'i temizlemek için sıkı doğrulanmalı.
            # Ortada kırmızı latch yokken zayıf/uzak yeşil görmek aracı STOP'a zorlamamalı.
            green_latch_active = (
                bool(tl.get("red_light_latched", False))
                or time.time() < float(self.red_light_latch_until or 0.0)
            )
            ...
            if reject_reasons:
                ...
                return {
                    ...
                    "risk": "LOW",
                    "target_speed": self.default_go_speed,
                    "reason": "green_light_weak_not_latched_release:" + ",".join(reject_reasons),
                }
